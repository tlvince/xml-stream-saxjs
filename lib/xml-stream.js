    ...
    if (event.type === 'updateElement') {
      ...
      this._fa.on('leave', finalState, function(element, context, trace) {
        ...
        if (!--this._bufferLevel && this._emitData) {
           emitElement.call(this, element, this._name, true);
        }
      });
    } else {
...
function parse() {
  ...
  xml.on('opentag', function (node) {
    ...
    if (this._bufferLevel === 0 && this._emitData) {
      emitStart.call(this, name, element.$);
    }
  });
  ...
  xml.on('closetag', function(name) {
    ...
    if (this._bufferLevel > 0 || this._preserveLevel > 0) {
      element.$name = name;
    }
